...
def remove_outliers(pcd, voxel_size_factor=0.05, k=10, threshold_multiplier=1.0):
    if len(pcd.points) < k + 1:
        return pcd
    voxel_size = np.mean(pcd.get_max_bound() - pcd.get_min_bound()) * voxel_size_factor
    voxel_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(pcd, voxel_size=voxel_size)
    points = np.asarray(pcd.points)
    voxel_indices = np.array([tuple(voxel_grid.get_voxel(p)) for p in points])
    voxel_density = {}
    unique_voxels, counts = np.unique(voxel_indices, axis=0, return_counts=True)
    for voxel, count in zip(unique_voxels, counts):
        voxel_density[tuple(voxel)] = count
    ...
    for i in range(len(points)):
        ...
        density = voxel_density.get(tuple(voxel_indices[i]), 1)
        if density > 0:
            deltas[i] = avg_distance / density
        else:
            deltas[i] = float('inf')
    if len(deltas) == 0: return pcd
...
def main():
    ...
    noise_levels = [20] # Use one level for focused interactive tuning
    ...
    for noise_level in noise_levels:
        ...
        pcd_outliers_removed = remove_outliers(pcd_noisy, voxel_size_factor=0.05, k=15, threshold_multiplier=1.0)
        ...
        print("2. Removing confounding points...")
        pcd_confounding_removed = remove_confounding_points(pcd_outliers_removed, k=20)
        pcd_confounding_removed.paint_uniform_color([0, 0, 1])
        print(f"  Points after confounding point removal: {len(pcd_confounding_removed.points)}")
        ...
        calculate_chamfer_distance(pcd_holes_filled, pcd_clean_copy)

        print(f"Displaying final comparison for {noise_level}% noise:")
        pcd_clean_copy.paint_uniform_color([0.5, 0.5, 0.5])
        pcd_noisy.paint_uniform_color([1, 0, 0])
        pcd_holes_filled.paint_uniform_color([0, 1, 0]) # Final result in Green
